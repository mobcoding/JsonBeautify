...
- [ ] 本地 `edge://extensions` 「加载解压缩」后，**首次安装**：
  ...
  - [ ] 切到「树状视图」节点可折叠、可展开；
  - [ ] 「工具集」页的转义/反转义、按 Key 排序、两份 JSON 比较均可用；
...
| 简短描述 Short description（180 字符内） | `简洁高效的 JSON 格式化、美化、压缩、验证与树状查看器。自动识别原始 JSON 页面并美化。全部处理本地完成，零数据上传。` |
| 详细描述 Long description（10k 字符内） | 复制下方 §3.2 长描述 |
| 分类 Category | Developer Tools |
| 子分类 Sub-Category | Web Development |
| 搜索关键词 Search terms | `json formatter`、`json beautify`、`json viewer`、`json validator`、`json tree`、`格式化`、`美化`、`压缩`、`开发者工具` |
| 目标设备 | 桌面（Desktop）即可 |
...
核心功能：
● 编辑器：粘贴/输入 JSON，一键美化格式化 / 压缩单行 / 语法验证；支持 2/4 空格缩进或 Tab，支持按键名（Key）递归排序，支持带注释、尾逗号、无引号 Key 等「松弛 JSON」自动修复。
● 统计面板：格式化后展示行数、字节数、对象键数、最大嵌套深度。
● 一键复制 / 下载 .json / 把结果再次送到输入区，方便迭代编辑。
● 树状视图：嵌套对象/数组按可折叠节点展示，点击叶子值直接复制，自定义默认展开层级、一键展开/折叠全部。
● 工具集：
  — 字符串转义（" → \"）与反转义；
  — 递归按 Key 排序；
...
● 右键菜单：
  ...
  — 验证选中的 JSON；
  — 复制美化后的 JSON。
● 自动识别原始 JSON 页面：访问以 .json 结尾的 URL、响应 Content-Type 为 application/json、或整页仅有 <pre> 的页面时，自动以暗色主题 + 语法高亮渲染，并提供原始/美化切换、一键复制、下载文件等浮层控件。
● 侧栏支持：长时间调试时可以把编辑器常驻在浏览器侧栏。

权限使用说明：
● storage：仅在本机浏览器保存用户偏好（缩进方式、展开层级、是否自动美化等），不会上传也不跨设备同步。
...
| `screenshot-2-tree-1280x800.png` | 截图 2 | 同上 | 树状视图可折叠节点 |
| `screenshot-3-auto-1280x800.png` | 截图 3 | 同上 | 访问原始 JSON 接口时自动美化 |
| `large-promo-1400x560.png` | 大型促销横幅 Large Promotional Tile | **必须** 1400×560 | 紫蓝渐变 + 代码卡片预览 |
...
#### `sidePanel`
> 允许用户在浏览器侧栏常驻 JSON 编辑器，便于长时间调试接口与编辑 JSON，无需反复打开 popup。仅当用户主动点击工具栏图标或面板内链接时打开侧栏，不会自动弹出。

#### `storage`
> 仅用于将用户的偏好设置（缩进方式、是否默认按键排序、展开层级、是否启用自动美化、是否启用语法高亮等）保存在本机浏览器的 `chrome.storage.local`。不与任何服务器同步，不上传任何数据。
...
- [ ] Manifest 中的 action / sidePanel / options_page 路径全部真实存在，不会 404；
